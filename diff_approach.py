...
def compare_row_counts(table_results,results,test_execution_id, source_df, target_df, source_table, target_table):
    ...
    table_results.append([source_table, "Row Count Comparison", execution_details])
...
def compare_row_by_row_sorted_chunks_nested_for(table_results,results,test_execution_id, source_df, target_df, source_table, target_table, chunksize=50000, id_columns=None):
    """
    ...
    try:
        ...
        if key_column in target_df.columns:
            ...
            if not duplicate_rows.empty:
                table_results.append([target_table, "Duplicate Data", f"Duplicate {key_column}={duplicate_rows[key_column].iloc[0]}"])
                ...
                results.append(prepare_test_execution_result(test_execution_id,3, "Duplicate Data", target_table, "Failed", f"{len(duplicate_rows)}_Duplicates found."))
        ...
        if not missing_rows.empty:
            rule = f"RowMissingInTarget (Target Table: {source_table})"
            result = f"{key_column}={missing_rows[key_column].iloc[0]}"
            table_results.append([source_table, rule, result])
            results.append(prepare_test_execution_result(
                test_execution_id, 4, "Missing Rows in Target", target_table, "Failed",
                f"{len(missing_rows)} rows missing in target."
            ))
            ...
            for col in source_columns:
                ...
                if not mismatched_col.empty:
                    for row_id, src, tgt in zip(
                        ...
                        rule = f"DataMismatch - {col}"
                        result = f"RowID: {row_id}, Source: {src}, Target: {tgt}"
                        mismatch_records.append([source_table, rule, result])

        # Save mismatches to a file if any are found
        if mismatch_records:
            execution_status = "Failed"
            execution_details = f"Mismatches found and {len(mismatch_records)} mismatches found."
            table_results.extend(mismatch_records)  # Append mismatches to the results list
        else:
...
def process_single_table_pair(args):
    ...
    try:
        ...
        # Compare row counts
        compare_row_counts(table_results,results,test_execution_id, source_df, target_df, source_table, target_table)
        ...
        # Save consolidated results to a single file for the table
        consolidated_file = os.path.join(output_folder, f"{source_table}_results_runid_{test_execution_id}.csv")
        pd.DataFrame(table_results, columns=['table_name', 'rule', 'result']).to_csv(consolidated_file, index=False)
        end_time = time.time()
